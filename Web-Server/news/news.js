...
class News {
  ...
  async create(data, id, imageName) {
    ...
    totalData.push({
      ...
      id,
      desc,
      thumbnail: `http://192.168.1.5:3000/${imageName}`,
    });
    await fs.promises.writeFile(this.path, JSON.stringify(totalData, null, 2));
    //await fs.promises.writeFile(this.path, JSON.stringify(totalData, null, 2));
    //db.insertDB(data.title ,data.content ,data.category, id, `http://192.168.1.5:3000/${imageName}`);
  }
